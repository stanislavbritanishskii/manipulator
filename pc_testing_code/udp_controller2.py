...
class UdpSliderUI:
	def __init__(self, ip, port):
		...
		self.root = tk.Tk()
		self.root.title("UDP Arm Control (7x uint8)")

		self.values = [tk.IntVar(value=init) for (_, init) in FIELDS]
		self.scales = []

		self.link_fingers = tk.IntVar(value=0)
		self.finger_offset = tk.IntVar(value=0)
		...
		# --- Finger link controls ---
		link_row = tk.Frame(self.root)
		link_row.pack(fill="x", padx=10, pady=6)

		link_chk = tk.Checkbutton(
			link_row,
			text="Move fingers together (mirrored)",
			variable=self.link_fingers,
			command=self._on_slider_change
		)
		link_chk.pack(side="left")

		offset_row = tk.Frame(self.root)
		offset_row.pack(fill="x", padx=10, pady=6)

		offset_lbl = tk.Label(offset_row, text="Finger offset", width=16, anchor="w")
		offset_lbl.pack(side="left")
	...
	def get_packet_bytes(self):
		...
		# first 5 joints
		for i in range(5):
			out[i] = self._clamp(int(self.values[i].get())) & 0xFF

		f1 = int(self.values[5].get())
		f2 = int(self.values[6].get())

		if self.link_fingers.get():
			offset = int(self.finger_offset.get())
			f2 = (180 - f1) + offset

		out[5] = self._clamp(f1) & 0xFF
		out[6] = self._clamp(f2) & 0xFF
